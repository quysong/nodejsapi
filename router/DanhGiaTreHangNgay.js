...
DanhGiaTreHangNgayController=function(req,res){
    ...
    switch (request.act) {
        case "GetAll_tb_ITIHCM_TinTuc":
        GetAll_tb_ITIHCM_TinTuc(res);
            break;
        case "GetAll":
        GetAll(res);
            break;
    ...
    if(_id)
        {
            var query =util.format(
                "delete from tbDanhGiaTreHangNgay "+
                "where ID='%s'",
                _id);
...
function AddRecord(res,_request){
    if(_request)
        {
            var query =util.format(
            "INSERT INTO tbDanhGiaTreHangNgay([hoten],[tinhtrangsuckhoe],[trangthai],[kienthuckynang]) "+
            "VALUES (N'%s',N'%s',N'%s',N'%s')",
            _request.hoten,_request.tinhtrangsuckhoe,_request.trangthai,_request.kienthuckynang);
...
function UpdateRecord(res,_request){
    var hoten=_request.hoten;
    var tinhtrangsuckhoe=_request.tinhtrangsuckhoe;
    var trangthai=_request.trangthai;
    var kienthuckynang=_request.kienthuckynang;
    ...
    var query =util.format(
        "UPDATE tbDanhGiaTreHangNgay "+
        "SET hoten = N'%s',tinhtrangsuckhoe = N'%s', trangthai=N'%s' ,"+
        "kienthuckynang=N'%s' "+
        "WHERE id='%s'",
        hoten,tinhtrangsuckhoe,trangthai,kienthuckynang,id);
...
async function GetAll_tb_ITIHCM_TinTuc(res){
    try {
        var query='select top 10 * from tb_test1';
        var result =  await executeQuery.AsyncQuery(query);
        res.json(result);
    } catch (error) {
        res.json(error);
...
async function GetAll(res){
    try {
        ...
        var query='select * from tbDanhGiaTreHangNgay';
        var result =  await executeQuery.AsyncQuery(query);
        res.json(result);
    } catch (error) {
        res.json(error);
...
async function GetRecordById(res,_request){
    try {
        var _id=_request.id;
        if(_id){
            var query=util.format("select * from tbDanhGiaTreHangNgay where id='%s'",_id);
            var result =  await executeQuery.AsyncQuery(query);
...
async function GetUploadImg(res,_request){
    try {
        var fileupload=_request.fileupload;
        if(fileupload){
...
module.exports = DanhGiaTreHangNgayController